...
//stephan: da die knoten irgendwie ueber tbb mit ttl=1 senden und ebenfalls eine ogm (vermutlich vom primar interface) mit ttl 50 (gleiche IPs) kommt, muss ich hier auch
//eine hoehere ttl beruecksichtigen, da sonst nur an das sendende interface die ogm zurueck gesendet wird (als bestaetigung) "Unidirect"
//max_othr_longtm_val und max_othr_recent_val in der schleife durch alle knoten ermittelt. wenn da schon mal ein ogm mit ttl 1 einen wert eingetragen hat, dann
//wuerde die ogm mit ttl 50 mit in den bereich der hysterese fallen und es wuerde das flag IS_BEST_NEIGH_AND_NOT_BROADCASTED nicht gesetzt und somit das ogm mit diesem ttl nicht weitergesendet
//so wie es aussieht koennte ich hier wieder die ttl speichern und dann mit der neuen ogm_ttl vergleichen und in der schleife nur nodes beachten die den gleichen
//ttl wert wie der der ogm haben und dann den max und resent berechnen.

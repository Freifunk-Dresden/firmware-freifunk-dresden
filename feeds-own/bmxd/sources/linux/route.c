...
//stephan: da ein knoten eimal fuer tbb und auch fuer das pimaer interface eine ogm sendet,kann es passieren dass die ogm von ttb mit ttl 1
//zuerst bearbeitet wird. das hat zur folge, dass fuer diese ogm/node die metric daten und seqno gespeichert werden. da ttl 1 ist, wird die
//ogm dann nicht weitergeleitet zum uebernachsten knoten und die ip taucht dann bei entfernten hops teilweise nicht mehr auf. folglich gibt
//es keine route zum knoten.
//wenn dann die ogm vom primar interface ttl 50 folgt, wird diese ignoriert, da ersten die seqno gleich ist und ebenfalls die metric daten in
//in die hysterese passen und somit keine routing aenderung auslosen
//es scheint egal zu sein, ob ich ueberall ttl als weiteres kriterium fuer die tests (seqno und metric: longterm/resent...) einbaue oder
//den ttl fuer alle interfaces gleich setzte. der ttl=1 war default so gesetzt, da die ip von den tbb aus einem 172er netz war und nicht 
//verteilt werden sollte. jetzt wo aber die ips gleich sind von tbb und primar interface kommen die ogms und deren seqno/metric in konflikt
        bif->if_ttl = 1;
        //bif->if_ttl = Ttl;

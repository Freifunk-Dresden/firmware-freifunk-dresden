...
void debug_send_list( struct ctrl_node *cn ) {
	...
	list_for_each( list_pos, &send_list ) {
		...
		dbg_printf( cn, "%-15s   (seqno %5d  ttl %3d)  at %u (if_seqno=%d) to iff %s\n", 
		         ipStr( ogm->orig ), ntohs(ogm->ogm_seqno), ogm->ogm_ttl, send_node->send_time, send_node->if_outgoing->if_seqno, send_node->if_outgoing->dev );
	}
...
/* stephan:
 ...
 Da ich aber nur eine ip pro freifunk knoten nutzen will und fuer die tbb interfaces somit ebenfalls
 die gleich ip habe, kommt es hier zum konflik.
 Die empfangende node bekommt einmal die grosse ogm (prim ogm) und dann noch die kleinen (link bezogen, ttl=1)
 ogm. da diese OGM die gleiche IP und seqno haben, lassen sie sich nicht mehr utnerscheiden.
 wenn die kleine ogm zuerstkommt (ohne hna,gw,...) wird diese wegen ttl=1 nur noch ein hop weiter geschickt.
 der übernächste hop sieht diesen freifunk knoten schon garnicht mehr. 
 Zusätzlich weiss der empfangene knoten nichts ueber hna oder gw, die der sendende knoten evt hat und somit
 ist auf dem empfangen knoten zb. das gw nicht verfuegbar (obwohl eigentlich vorhanden).
 Da die OGMs mal schneller oder langsamer sind und ueber verschiedene interfaces und wege eintreffen,
 kann es passieren, dass die reihenfolge diese grossen und kleinen ogms wechseln. resultat ist,
 dass mal ein gw da ist und mal nicht, und ebenso taucht der freifunk knoten am übernächsten hop mal auf oder
 verschwindet wieder.
 Die meiste zeit kam die grosse ogm zuerst an, so dass das netz noch funktioniert, aber es kommt zu oft vor
 dass es andersrum ist.

 meine lösung ist die grosse ogm, welche den knoten representiert nicht mehr "extra verschicke".
 diese fuer das primar itnerface erzeugte ogm wird also nicht mehr gesendet (aber noch scheduled).
 ebenso senden alle non-prim interfaces die grosse ogm statt nur der kleinen.
 die primar ogm muss ich unterbinden, da der empfänger schon ein teil seiner metric damit neu
 berechnet, bevor diese ogm als "schon gesehen" verworfen wird.

 das scheint ganz gut zu funktionieren. aber es gibt noch probleme, dass mal ein knoten ogms
 garnicht verschickt. 
 das passiert wenn das unidirectflag gesetzt ist und aus irgendeinem grund die variable 
 "iteration" auf 2 steht (groesser alls if_ant_diversity==1).
 
 Wenn OGM von einem anderen node ueber mehrere interfaces kommt, werden diese in der send_list
 fuer das empfangene itnerface eingehängt. bei WLAN kann es passieren dass gleich OGM von zwei
 benachberten freifunk knoten ueber das selbe wlan interface kommen.
 Dann gibt es zwei listen einträge der gleichen node (send_node) und dem gleichen interface.
 in diesem fall könnte interation auch grösser werden als 1.
 Hier könnte der fehler liegen, dass bei unidirectflag iteration nicht beruecksichtigen darf
 und die ogm im ersten if-zweig an das interface gehängt werden muss, damit es später
 gesendet wird.
 evt. muss ich fuer das wlan interface "if_ant_diversity" beim bmxd start konfigurieren
 und auf was anderes stellen als 1. Also auf die mögliche anzahl von direkten nachbarn.
...
#else
        int sn_size = sizeof (struct send_node) + ( MAX_UDPD_SIZE + 1 );
	if (  bif == primary_if  ) {
		dbgf_all( DBGT_INFO, "## DONT SEND OGM for primary interface");
//		return;
	}
#endif

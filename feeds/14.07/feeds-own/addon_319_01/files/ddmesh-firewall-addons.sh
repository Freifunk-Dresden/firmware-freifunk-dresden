...
setup_splash() {
	...
	$IPT -A forwarding_rule -i $wifi2_device -s $WIFIADR/$WIFIPRE -j SPLASH
}
...
callback_add_ignored_nodes() {
	$IPT -A input_ignore_nodes -s $1 -j DROP
}
...
setup_ignored_nodes() {
	...
	config_load ddmesh
	config_list_foreach ignore_nodes ip callback_add_ignored_nodes
}

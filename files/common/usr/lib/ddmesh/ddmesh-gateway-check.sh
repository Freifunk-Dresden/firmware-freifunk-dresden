...
network_is_up cwan  && {
	#get network infos using /lib/functions/network.sh
	network_get_device default_cwan_ifname cwan
	default_cwan_gateway=$(ip route | sed -n "/default via [0-9.]\+ dev $default_cwan_ifname/{s#.*via \([0-9.]\+\).*#\1#p}")
	if [ -n "$default_cwan_gateway" -a -n "$default_cwan_ifname" ]; then
		cwan_default_route="$default_cwan_gateway:$default_cwan_ifname"
	fi
}
printf 'CWAN:%s via %s\n' "$default_cwan_ifname" "$default_cwan_gateway"
...
for g in $vpn_default_route $wan_default_route $lan_default_route $cwan_default_route $twan_default_route $wwan_default_route
do
	printf '===========\n'
	printf 'try: %s\n' "$g"
	dev="${g#*:}"
	...
	if [ "$ok" = "1" ]; then
		...
		printf 'landev: [%s], wandev=[%s], vpndev=[%s], twan=[%s], wwan=[%s]\n' "$default_lan_ifname" "$default_wan_ifname" "$default_vpn_ifname" "$default_cwan_ifname" "$default_twan_ifname" "$default_wwan_ifname"

		#always add wan/wwan or lan to local gateway
		if [ "$dev" = "$default_lan_ifname" -o "$dev" = "$default_wan_ifname" -o "$dev" = "$default_cwan_ifname" -o "$dev" = "$default_twan_ifname" -o "$dev" = "$default_wwan_ifname" ]; then

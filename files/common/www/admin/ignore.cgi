...
print_node() {
	...
	if [ -n "$1" ]; then
		...
		echo "<td><input disabled name="form_opt_wifi" type="checkbox" value="1" $(if [ "$opt_wifi" = "1" ];then echo 'checked="checked"';fi)></td>"
...
<table>
 ...
 <tr>	<td><input name="form_node" type="text" value="" size="17" maxlength="17"></td>
	<td><input name="form_opt_lan" type="checkbox" value="1" $(if [ "$lan" = "1" ];then echo 'checked="checked"';fi)></td>
	<td><input name="form_opt_tbb" type="checkbox" value="1" $(if [ "$tbb" = "1" ];then echo 'checked="checked"';fi)></td>
	<td><input name="form_opt_wifi" type="checkbox" value="1" $(if [ "$wifi" = "1" ];then echo 'checked="checked"';fi)></td>
	...
	if [ -n "$form_action" ]; then
		case $form_action in
		  add)
			...
			entry="$node:$form_opt_lan:$form_opt_tbb:$form_opt_wifi"
			uci add_list ddmesh.ignore_nodes.node="$entry"
			uci_commit.sh
			notebox "Knoten <b>$node</b> wurde zur Konfiguration hinzugef&uuml;gt. Bitte Konfiguration aktivieren!"
			;;
		  del)
			node=$(uhttpd -d $form_node)
			uci del_list ddmesh.ignore_nodes.node="$node"
			uci_commit.sh
			notebox "Knoten <b>$node</b> wurde gel&ouml;scht. Bitte Konfiguration aktivieren!"
			;;
		  delall)
			uci delete ddmesh.ignore_nodes.node
			uci_commit.sh
			notebox "Alle Knoten wurden gel&ouml;scht. Bitte Konfiguration aktivieren!"
			;;
		  firewall_update)

#!/bin/sh
. /www/page-functions.sh

eval $(/usr/lib/ddmesh/ddmesh-utils-network-info.sh wifi)

#check if access comes from WAN interface and we have access to "Verwalten" enabled
...
test ! "$(uci get ddmesh.system.wifisetup)" = "1" && test "$in_ifname" = "$net_device" && enable_setup=0
test ! "$(uci get ddmesh.system.wifisetup)" = "1" && test "$in_ifname" = "$net_device" && enable_setup=0
...
<TR><TD COLSPAN="5">
 <TABLE WIDTH="100%" BORDER="0" CELLPADDING="0" CELLSPACING="0">
  ...
  <TD HEIGHT="33" style="vertical-align: middle;"><font size="5"><b>$COMMUNITY</b>&nbsp;$_ddmesh_node</font></TD>

...
run_ramfs() { # <command> [...]
	install_bin /bin/busybox /bin/ash /bin/sh /bin/mount /bin/umount	\
		/sbin/pivot_root /bin/uclient-fetch /sbin/reboot /bin/sync /bin/dd	\
	...
	for stat in /proc/[0-9]*/stat; do
		...
		if [ $$ -eq 1 ] || [ $my_ppid -eq 1 ] && [ -n "$my_ppisupgraded" ]; then
			...
			case "$name" in
				...
				*procd*|*ash*|*init*|*watchdog*|*ssh*|*dropbear*|*telnet*|*login*|*hostapd*|*wpa_supplicant*|*nas*|*uhttpd*) : ;;
...
get_image() { # <source> [ <command> ]
	...
	case "$from" in
		http://*|ftp://*) cmd="uclient-fetch -O- -q";;
		*) cmd="cat";;
	esac
	if [ -z "$conc" ]; then
